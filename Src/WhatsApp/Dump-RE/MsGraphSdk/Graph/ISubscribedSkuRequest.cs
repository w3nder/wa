// Decompiled with JetBrains decompiler
// Type: Microsoft.Graph.ISubscribedSkuRequest
// Assembly: MsGraphSdk, Version=1.1.0.0, Culture=neutral, PublicKeyToken=null
// MVID: B6767127-13D0-4992-B741-2642C0E7F410
// Assembly location: C:\Users\Admin\Desktop\RE\WABeta\MsGraphSdk.dll

using System.Threading;
using System.Threading.Tasks;

#nullable disable
namespace Microsoft.Graph
{
  public interface ISubscribedSkuRequest : IBaseRequest
  {
    Task<SubscribedSku> CreateAsync(SubscribedSku subscribedSkuToCreate);

    Task<SubscribedSku> CreateAsync(
      SubscribedSku subscribedSkuToCreate,
      CancellationToken cancellationToken);

    Task DeleteAsync();

    Task DeleteAsync(CancellationToken cancellationToken);

    Task<SubscribedSku> GetAsync();

    Task<SubscribedSku> GetAsync(CancellationToken cancellationToken);

    Task<SubscribedSku> UpdateAsync(SubscribedSku subscribedSkuToUpdate);

    Task<SubscribedSku> UpdateAsync(
      SubscribedSku subscribedSkuToUpdate,
      CancellationToken cancellationToken);
  }
}
