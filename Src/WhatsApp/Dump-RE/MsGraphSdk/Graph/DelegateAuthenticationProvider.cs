// Decompiled with JetBrains decompiler
// Type: Microsoft.Graph.DelegateAuthenticationProvider
// Assembly: MsGraphSdk, Version=1.1.0.0, Culture=neutral, PublicKeyToken=null
// MVID: B6767127-13D0-4992-B741-2642C0E7F410
// Assembly location: C:\Users\Admin\Desktop\RE\WABeta\MsGraphSdk.dll

using System.Net.Http;
using System.Threading.Tasks;

#nullable disable
namespace Microsoft.Graph
{
  public class DelegateAuthenticationProvider : IAuthenticationProvider
  {
    public DelegateAuthenticationProvider(
      AuthenticateRequestAsyncDelegate authenticateRequestAsyncDelegate)
    {
      this.AuthenticateRequestAsyncDelegate = authenticateRequestAsyncDelegate;
    }

    public AuthenticateRequestAsyncDelegate AuthenticateRequestAsyncDelegate { get; set; }

    public Task AuthenticateRequestAsync(HttpRequestMessage request)
    {
      return this.AuthenticateRequestAsyncDelegate != null ? this.AuthenticateRequestAsyncDelegate(request) : (Task) Task.FromResult<int>(0);
    }
  }
}
