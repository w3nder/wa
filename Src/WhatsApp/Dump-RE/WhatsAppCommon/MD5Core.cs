...
public sealed class MD5Core
{
  ...
  public static byte[] GetHash(string input, Encoding encoding)
  {
    ...
    return encoding != null ? MD5Core.GetHash(encoding.GetBytes(input)) : throw new ArgumentNullException(nameof (encoding), "Unable to calculate hash over a string without a default encoding. Consider using the GetHash(string) overload to use UTF8 Encoding");
  }
  ...
  {
    return input != null ? BitConverter.ToString(MD5Core.GetHash(input)).Replace("-", "") : throw new ArgumentNullException(nameof (input), "Unable to calculate hash over null input data");
  }
  ...
  public static string GetHashString(string input, Encoding encoding)
  {
    if (input == null)
      throw new ArgumentNullException(nameof (input), "Unable to calculate hash over null input data");
    return encoding != null ? MD5Core.GetHashString(encoding.GetBytes(input)) : throw new ArgumentNullException(nameof (encoding), "Unable to calculate hash over a string without a default encoding. Consider using the GetHashString(string) overload to use UTF8 Encoding");
  }
  ...
  {
    return input != null ? MD5Core.GetHash(input, 0, input.Length) : throw new ArgumentNullException(nameof (input), "Unable to calculate hash over null input data");
  }
  ...
  public static byte[] GetHash(byte[] input, int offset, int length)
  {
    ...
    if (offset + length > input.Length)
      throw new InvalidOperationException("extent exceeds size of array");
    ...
    int ibStart;
    for (ibStart = offset; ibStart <= length - 64; ibStart += 64)
      MD5Core.GetHashBlock(input, ref ABCDValue, ibStart);
    return MD5Core.GetHashFinalBlock(input, ibStart, length - ibStart, ABCDValue, (long) length * 8L);
  }

  internal static byte[] GetHashFinalBlock(
    byte[] input,
    int ibStart,
    int cbSize,
    ABCDStruct ABCD,
    long len)
  {
    ...
    Array.Copy((Array) input, ibStart, (Array) numArray1, 0, cbSize);
    numArray1[cbSize] = (byte) 128;
    if (cbSize <= 56)
    {
      Array.Copy((Array) bytes, 0, (Array) numArray1, 56, 8);
      MD5Core.GetHashBlock(numArray1, ref ABCD, 0);
    }
    else
    {
      MD5Core.GetHashBlock(numArray1, ref ABCD, 0);
      byte[] numArray2 = new byte[64];
      Array.Copy((Array) bytes, 0, (Array) numArray2, 56, 8);
      MD5Core.GetHashBlock(numArray2, ref ABCD, 0);
    }
    byte[] destinationArray = new byte[16];
    Array.Copy((Array) BitConverter.GetBytes(ABCD.A), 0, (Array) destinationArray, 0, 4);
    Array.Copy((Array) BitConverter.GetBytes(ABCD.B), 0, (Array) destinationArray, 4, 4);
    Array.Copy((Array) BitConverter.GetBytes(ABCD.C), 0, (Array) destinationArray, 8, 4);
    Array.Copy((Array) BitConverter.GetBytes(ABCD.D), 0, (Array) destinationArray, 12, 4);
