        ...
        case JTokenType.Array:
          ...
          foreach (JToken jtoken in (IEnumerable<JToken>) token)
          {
            if (jtoken.Type != JTokenType.String)
              throw JsonException.Create((IJsonLineInfo) jtoken, jtoken.Path, "Exception JSON schema type string token, got {0}.".FormatWith((IFormatProvider) CultureInfo.InvariantCulture, (object) token.Type));
