...
// Type: ZXing.PDF417.Internal.PDF417ErrorCorrection
...
namespace ZXing.PDF417.Internal
{
  ...
  internal static class PDF417ErrorCorrection
  {
    ...
    internal static string generateErrorCorrection(string dataCodewords, int errorCorrectionLevel)
    {
      ...
      char[] chArray = new char[correctionCodewordCount];
      int length = dataCodewords.Length;
      for (int index1 = 0; index1 < length; ++index1)
      {
        int num1 = ((int) dataCodewords[index1] + (int) chArray[chArray.Length - 1]) % 929;
        for (int index2 = correctionCodewordCount - 1; index2 >= 1; --index2)
        {
          int num2 = 929 - num1 * PDF417ErrorCorrection.EC_COEFFICIENTS[errorCorrectionLevel][index2] % 929;
          chArray[index2] = (char) (((int) chArray[index2 - 1] + num2) % 929);
        }
        int num3 = 929 - num1 * PDF417ErrorCorrection.EC_COEFFICIENTS[errorCorrectionLevel][0] % 929;
        chArray[0] = (char) (num3 % 929);
      }
      StringBuilder stringBuilder = new StringBuilder(correctionCodewordCount);
      for (int index = correctionCodewordCount - 1; index >= 0; --index)
      {
        if (chArray[index] != char.MinValue)
          chArray[index] = (char) (929U - (uint) chArray[index]);
