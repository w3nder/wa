...
// Type: ICSharpCode.SharpZipLib.Silverlight.Zip.ZipFile
...
namespace ICSharpCode.SharpZipLib.Silverlight.Zip
{
  public class ZipFile : IEnumerable, IDisposable
  {
    ...
    public int FindEntry(string name, bool ignoreCase)
    {
      ...
      {
        if (name.Compare(this.entries_[entry].Name, ignoreCase, CultureInfo.InvariantCulture) == 0)
          return entry;
      }
      return -1;
    }
    ...
    public void BeginUpdate(IArchiveStorage archiveStorage, IDynamicDataSource dataSource)
    {
      ...
      if (this.entries_ != null)
      {
        ...
        {
          ZipFile.ZipUpdate zipUpdate = new ZipFile.ZipUpdate(entry);
          this.updates_.Add(zipUpdate);
          int num = this.updates_.IndexOf(zipUpdate);
          this.updateIndex_.Add(entry.Name, num);
        }
      }
      else
        this.updates_ = new List<ZipFile.ZipUpdate>();
    ...
    public void CommitUpdate()
    {
      ...
      try
      {
        ...
        this.updateIndex_ = (Dictionary<string, int>) null;
    ...
    private void AddUpdate(ZipFile.ZipUpdate update)
    {
      ...
      if (existingUpdate >= 0)
      {
        if (this.updates_[existingUpdate] == null)
          ++this.updateCount_;
        this.updates_[existingUpdate] = update;
      }
      else
      {
        this.updates_.Add(update);
        int num = this.updates_.IndexOf(update);
        ++this.updateCount_;
    ...
    public void Add(IStaticDataSource dataSource, string entryName)
    {
      if (dataSource == null)
        throw new ArgumentNullException(nameof (dataSource));
      this.CheckUpdating();
      this.AddUpdate(new ZipFile.ZipUpdate(dataSource, this.EntryFactory.MakeFileEntry(entryName)));
    }
    ...
    public bool Delete(string fileName)
    {
      ...
      bool flag = true;
      this.contentsEdited_ = true;
      this.updates_[existingUpdate] = (ZipFile.ZipUpdate) null;
      --this.updateCount_;
      return flag;
    }
    ...
    private int WriteCentralDirectoryHeader(ZipEntry entry)
    {
      ...
      byte[] buffer = entry.Comment != null ? Encoding.UTF8.GetBytes(entry.Comment) : new byte[0];
      if (buffer.Length > 0)
    ...
    private void PostUpdateCleanup()
    {
      ...
      this.updates_ = (List<ZipFile.ZipUpdate>) null;
      this.updateIndex_ = (Dictionary<string, int>) null;
    }

    private string GetTransformedFileName(string name)
    {
      return this.NameTransform == null ? name : this.NameTransform.TransformFile(name);
    }

    private string GetTransformedDirectoryName(string name)
    {
      return this.NameTransform == null ? name : this.NameTransform.TransformDirectory(name);
    }
    ...
    private int FindExistingUpdate(ZipEntry entry)
    {
      ...
      if (this.updateIndex_.ContainsKey(transformedFileName))
        existingUpdate = this.updateIndex_[transformedFileName];
    ...
    private void RunUpdates()
    {
      ...
      else if (this.archiveStorage_.UpdateMode == FileUpdateMode.Direct)
      {
        ...
        workFile.baseStream_.Position = 0L;
        flag2 = true;
        this.updates_.Sort((IComparer<ZipFile.ZipUpdate>) new ZipFile.UpdateComparer());
      }
      else
      {
    ...
    private class UpdateComparer : IComparer<ZipFile.ZipUpdate>
    {
      ...
      {
        ZipFile.ZipUpdate zipUpdate1 = x;
        ZipFile.ZipUpdate zipUpdate2 = y;
        int num1;
        if (zipUpdate1 == null)
